...
\begin{enumerate}\setlength{\itemsep}{2pt}
  \item \textbf{Data pipeline} --- curiosity to map in 20 min
  \item \textbf{Code \& paper audit} --- AI vs.\ real referees
\end{enumerate}
\vskip3pt
{\tiny\color{CoolGray} Multi-step workflows where you check each step.}
\end{tcolorbox}
\end{column}
\begin{column}{0.32\textwidth}
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[text=UVMGold, font=\normalsize]
    at ([yshift=-0.5cm]current page.center) {Data Pipelines \& Code Auditing};
\end{tikzpicture}
\end{frame}
}

%% ====== SLIDE: SESSION 1 RECAP + TODAY'S FRAME ======
...
\begin{tikzpicture}[
  ...
  \node[takeaway, anchor=west] (t1) at (-5.8,2.0) {Data pipeline: curiosity $\rightarrow$ map $\rightarrow$ memo in \textbf{20 minutes}};
  \pause
  \node[circle, fill=UVMGold, text=white, font=\scriptsize\bfseries, minimum size=0.5cm] (n2) at (-6.2,1.0) {2};
  \node[takeaway, anchor=west] (t2) at (-5.8,1.0) {Code audit + paper audit: AI matched \textbf{7 of 10} real referee criticisms};
  \pause
  \node[circle, fill=UVMGold, text=white, font=\scriptsize\bfseries, minimum size=0.5cm] (n3) at (-6.2,0.0) {3};
  \node[takeaway, anchor=west] (t3) at (-5.8,0.0) {Built and ran a real teaching skill --- \textbf{lecture-builder}};
  \pause
  \node[circle, fill=UVMGold, text=white, font=\scriptsize\bfseries, minimum size=0.5cm] (n4) at (-6.2,-1.0) {4};
  \node[takeaway, anchor=west] (t4) at (-5.8,-1.0) {Stress-tested a research idea with \textbf{evidence-grounded critique}};
  \pause
...
\onslide<6->{%
\begin{tcolorbox}[accentbox, width=11cm, halign=center, top=3pt, bottom=3pt]
\small Five workflows. Five artifacts on disk. \textbf{All reusable.}
\end{tcolorbox}
}
\end{frame}
