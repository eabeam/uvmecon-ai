...
\end{column}
\begin{column}{0.46\textwidth}
\begin{tcolorbox}[pipelinebox, title={\faFile\quad ``Literature'' (4 pts), rewritten}, top=2pt, bottom=2pt]
\scriptsize\ttfamily
\newcommand{\rind}{\hspace*{1.3em}}%
\textbf{gates} (deterministic):\\
\rind >= 4 peer-reviewed in lit review\\
\rind distinct from the 2 intro sources\\
\rind NBER / World Bank / gov't = not\\[3pt]
\textbf{checks} (agent -- reliable):\\
\rind every source actually discussed\\
\rind each one linked back to the RQ\\
\rind what's known vs. still unknown\\[3pt]
\textbf{judge} (you):\\
\rind synthesis, or summary?\\[3pt]
\textbf{score}: 4 = gates pass + all checks\\
\hspace*{3.9em} 3 = one check missed\\
\hspace*{3.9em} 0 = gate fail -> flag
\end{tcolorbox}
\vskip3pt
\begin{tcolorbox}[accentbox, top=2pt, bottom=2pt]
\scriptsize The original said ``important literature discussed.'' That's a judgment word doing a gate's job --- and it's the line every grader inflated.
\end{tcolorbox}
\end{column}
\end{columns}
\end{frame}
