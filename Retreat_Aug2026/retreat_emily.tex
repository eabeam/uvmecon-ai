...
\begin{enumerate}\setlength{\itemsep}{2pt}
  ...
  \item \textbf{Split check from judge.} Checks (are there four peer-reviewed sources? is there an equation with the variables defined?) are where AI is reliable. Judgment stays yours.
...
\end{tcolorbox}
\vskip1pt
\begin{center}{\color{CoolGray}\scriptsize $\downarrow$\ \ three different questions\ \ $\downarrow$}\end{center}
\vskip1pt
\begin{tcolorbox}[dobox, title={\faCheck\quad Countable --- make it a gate}, top=1pt, bottom=1pt, fonttitle=\bfseries\scriptsize]
\scriptsize Four peer-reviewed, distinct from the intro's two. A working paper isn't one.
\end{tcolorbox}
\vskip2pt
\begin{tcolorbox}[pipelinebox, title={\faSearch\quad Checkable --- hand it to the agent}, top=1pt, bottom=1pt]
\scriptsize Is each source actually discussed, not just cited? Tied back to the question?
\end{tcolorbox}
\vskip2pt
\begin{tcolorbox}[accentbox, top=1pt, bottom=1pt]
\scriptsize \textbf{\faBalanceScale\ \ Judgment --- yours.} Synthesis, or summary? \emph{Every grader inflated this one.}
\end{tcolorbox}
\end{column}
\end{columns}
\end{frame}
...
%% ====== SLIDE: WHAT IT TAKES TO TRUST IT ======
\begin{frame}{What grading with agents actually taught me}
\vskip-4pt
\begin{columns}[T]
\begin{column}{0.49\textwidth}
...
\begin{itemize}\setlength{\itemsep}{2pt}
  ...
  \item \textbf{Disagreement is the product.} Only the papers where the graders disagreed came to me.
\end{itemize}
\end{tcolorbox}
\end{column}
\begin{column}{0.49\textwidth}
...
\scriptsize
\begin{itemize}\setlength{\itemsep}{2pt}
  \item \textbf{Drift is up, and it's on the judgment lines.} You just saw it: four graders, four scores above the key, every gap on a criterion with an interpolated scale.
  \item \textbf{One pass is never enough.} Cursory $\rightarrow$ grade $\rightarrow$ audit $\rightarrow$ blind regrade $\rightarrow$ harshness audit. Hours to the first feedback document; more sessions to get it right.
...
\vskip3pt
\begin{tcolorbox}[accentbox, top=2pt, bottom=2pt]
\small Every correction I've had to make landed on a \emph{judgment} criterion with an interpolated scale. None landed on a gate or a check.
\end{tcolorbox}
\end{frame}
