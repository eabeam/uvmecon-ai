...
\small
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Too long arguing with Claude about the appearance of call-out boxes. 
  \item A JavaScript version of slides I got super excited about, and then lost my mouse in, mid-lecture
\end{itemize}
\end{tcolorbox}
\vskip4pt
\begin{tcolorbox}[accentbox, top=2pt, bottom=2pt]
...
\end{columns}
\vskip2pt
\begin{tcolorbox}[dontbox, title={\faBan\quad Not this path}, top=1pt, bottom=1pt]
...
\small
In a \textbf{small seminar}, an ``anonymized'' essay \emph{could} still be re-identifiable from:
\vskip2pt
\begin{itemize}\setlength{\itemsep}{1pt}
  \item topic choice
  \item writing style
...
%% ====== SLIDE: A RUBRIC AN AGENT CAN GRADE (1/2) ======
\begin{frame}{A rubric an agent can grade}
\vskip-2pt
\begin{columns}[T]
\begin{column}{0.32\textwidth}
\begin{tcolorbox}[pipelinebox, title={\faBook\quad Before you write it}, top=2pt, bottom=2pt]
\scriptsize
\begin{enumerate}\setlength{\itemsep}{4pt}
  ...
  \item \textbf{Show it your own feedback.} Real comments you wrote to real students, plus your grades on the earlier assignments. This is the difference between generic feedback and feedback that sounds like you --- and knows what you already told them.
\end{enumerate}
\end{tcolorbox}
\end{column}
\begin{column}{0.32\textwidth}
\begin{tcolorbox}[keybox, title={\faListOl\quad In the rubric itself}, top=2pt, bottom=2pt]
\scriptsize
\begin{enumerate}\setlength{\itemsep}{4pt}\setcounter{enumi}{2}
  ...
  \item \textbf{Split check from judge.} Checks are where AI is reliable. Judgment stays yours.
\end{enumerate}
\end{tcolorbox}
\end{column}
\begin{column}{0.32\textwidth}
\begin{tcolorbox}[dobox, title={\faFlask\quad Before you trust it}, top=2pt, bottom=2pt]
\scriptsize
\begin{enumerate}\setlength{\itemsep}{4pt}\setcounter{enumi}{5}
  \item \textbf{Do a test grade.} Run it on something you've already scored yourself --- last term's paper, or a fake one with flaws you planted. That's how you find out where it drifts, before it touches a real student.
  \item \textbf{Plan the second pass.} Budget it. You will recalibrate.
\end{enumerate}
\end{tcolorbox}
\end{column}
\end{columns}
...
%% ====== SLIDE: A RUBRIC AN AGENT CAN GRADE (2/2) ======
\begin{frame}{One rubric line, three different questions}
\vskip2pt
\begin{center}
\begin{tcolorbox}[quotebox, width=0.93\textwidth, top=3pt, bottom=3pt]
\small
\textit{``Important literature discussed and linked to the RQ; at least four peer-reviewed sources distinct from the intro.''}\hfill\textbf{4 pts}
\end{tcolorbox}
\end{center}
\vskip3pt
\begin{center}{\color{CoolGray}\footnotesize $\downarrow$\qquad one line, but three different kinds of question\qquad $\downarrow$}\end{center}
\vskip4pt
\begin{columns}[T]
\begin{column}{0.32\textwidth}
\begin{tcolorbox}[dobox, title={\faCheck\quad Countable}, top=2pt, bottom=2pt]
\scriptsize
Four peer-reviewed, distinct from the intro's two. A working paper isn't one.
\vskip3pt
\textbf{Make it a gate.} Same answer every time, from anyone.
\end{tcolorbox}
\end{column}
\begin{column}{0.32\textwidth}
\begin{tcolorbox}[pipelinebox, title={\faSearch\quad Checkable}, fonttitle=\bfseries\small, top=2pt, bottom=2pt]
\scriptsize
Is each source actually discussed, not just cited? Tied back to the question? Does it say what's still unknown?
\vskip3pt
\textbf{Hand it to the agent.} This is what it's reliably good at.
\end{tcolorbox}
\end{column}
\begin{column}{0.32\textwidth}
\begin{tcolorbox}[accentbox, title={\faBalanceScale\quad Judgment}, fonttitle=\bfseries\small, coltitle=DeepCharcoal, colbacktitle=LightGold!80, top=2pt, bottom=2pt]
\scriptsize
Synthesis, or summary?
\vskip3pt
\textbf{Keep it.} And watch it --- this is the line they disagree on most.
\end{tcolorbox}
\end{column}
\end{columns}
\end{frame}
...
%% ====== SLIDE: WHAT CAME IN ======   (CUT 2026-08-26 by Emily -- fake example superseded by the real one; \iffalse ... \fi to restore)
\iffalse
...
\begin{itemize}\setlength{\itemsep}{3pt}
  ...
  \item \textbf{14 flaws planted on purpose}, each mapped to a rubric line: a lit review that summarizes instead of synthesizes, a working paper counted as peer-reviewed, ``this will \emph{prove}\,'' where it should hedge, a table with no group split, no clustering mentioned.
  \item \textbf{Answer key written first}, before any grader saw it. Target: \textbf{28.5/38} (Real class: median 31.25.)
\end{itemize}
\end{tcolorbox}
...
\begin{frame}{How I run things --- four passes, one command}
\vskip-4pt
\begin{columns}[T]
\begin{column}{0.40\textwidth}
\begin{center}
...
%% ====== SLIDE: FOUR GRADERS, ONE PROPOSAL ======   (CUT 2026-08-26 by Emily -- fake example superseded by the real one; \iffalse ... \fi to restore)
\iffalse
\begin{frame}{Five graders, one proposal}
\vskip-8pt
{\footnotesize A fake ECON 3500 proposal with 14 planted flaws. My real Spring 2026 grading prompt. Target: 28.5/38.\par}
\vskip2pt
\begin{columns}[T]
...
\begin{tabular}{>{\raggedright\arraybackslash}m{2.05cm} c c c c c c}
\rowcolor{SlateTeal}
\textcolor{white}{\textbf{Component (pts)}} & \textcolor{white}{\textbf{Key}} & \textcolor{white}{\textbf{Copilot}} & \textcolor{white}{\textbf{Fable}} & \textcolor{white}{\textbf{Sonnet}} & \textcolor{white}{\textbf{Codex}} & \textcolor{white}{\textbf{Opus}} \\
\rowcolor{PaleSage} Introduction (6) & 4.5 & 6 & 6 & 6 & 6 & 6 \\
\rowcolor{white}    Literature (4) & 2 & 3 & \textbf{2} & 3 & \textbf{2} & \textbf{2} \\
\rowcolor{PaleSage} Data + table (6) & 5 & 6 & \textbf{5} & 6 & \textbf{5} & \textbf{5} \\
\rowcolor{white}    Empirical (10) & 7 & 7.5 & 8 & 8 & 7.5 & 8 \\
\rowcolor{PaleSage} Threats (2) & 1.5 & 2 & 2 & 2 & 2 & 2 \\
\rowcolor{white}    Presentation (10) & 8.5 & 9.5 & 9.5 & 9.5 & 9.5 & \textbf{8.5} \\
\rowcolor{LightGold} \textbf{Total (38)} & \textbf{28.5} & \textbf{34} & \textbf{32.5} & \textbf{34.5} & \textbf{32} & \textbf{31.5} \\
\rowcolor{white}    Noticed (of 14) & --- & 7$^{*}$ & 13 & 8 & 11 & 13 \\
\rowcolor{PaleSage} Deducted for & --- & 6$^{*}$ & 10 & 6 & 9 & \textbf{12} \\
\rowcolor{white}    Flaws invented & --- & 0 & 0 & 0 & 0 & 0 \\
\end{tabular}
}
\vskip4pt
{\scriptsize\color{CoolGray} Copilot (UVM NetID) $\cdot$ Fable 5, Sonnet 5, Opus 5 as fresh blind agents $\cdot$ Codex (GPT-5.6-sol, high) by hand.\\
$^{*}$Copilot is a floor: only its score breakdown was captured.\par}
\end{column}
\begin{column}{0.44\textwidth}
\begin{tcolorbox}[keybox, title={\faSearch\quad What happened}, top=2pt, bottom=2pt]
\small
\begin{itemize}\setlength{\itemsep}{3pt}
  \item \textbf{Everyone drifted up} --- always on the judgment criteria. But not equally: Opus $+3$, Sonnet $+6$.
  \item \textbf{The checks held.} Both overclaims, the missing reference, the source count: all five.
  \item \textbf{The gap is ``noticed'' vs.\ ``deducted.''} Sonnet saw 8 and cut for 6. Opus saw 13 and cut for 12.
  \item \textbf{Nobody invented anything.} Zero hallucinated defects, all five.
\end{itemize}
\end{tcolorbox}
%\vskip4pt
%\begin{tcolorbox}[accentbox, top=1pt, bottom=1pt]
%\scriptsize \textbf{Read the notes, not the numbers.} And one planted flaw \textbf{nobody caught} --- all five said the opposite.
%\end{tcolorbox}
\end{column}
\end{columns}
\end{frame}
...
%% ====== SLIDE: FIVE GRADERS, ONE REAL PROPOSAL ======
\begin{frame}{Five graders, one real proposal}
\vskip-6pt
{\footnotesize A real S26 submission, de-identified, mid-distribution. My actual rubric and prompt, all five blind.\par}
\vskip1pt
\begin{columns}[T]
\begin{column}{0.58\textwidth}
{\scriptsize
\setlength{\tabcolsep}{2.2pt}\renewcommand{\arraystretch}{0.98}
\begin{tabular}{>{\raggedright\arraybackslash}m{2.45cm} c c c c c c}
\rowcolor{SlateTeal}
\textcolor{white}{\textbf{Criterion}} & \textcolor{white}{\textbf{Me}} & \textcolor{white}{\textbf{Cop.}} & \textcolor{white}{\textbf{Son.}} & \textcolor{white}{\textbf{Opus}} & \textcolor{white}{\textbf{Fable}} & \textcolor{white}{\textbf{5.6}} \\
\rowcolor{white}     Research question (2) & 2 & 2 & 2 & 1.5 & 2 & 2 \\
\rowcolor{PaleSage}  Motivation (4) & 4 & 4 & 3 & 3 & 3 & 3 \\
\rowcolor{white}     Literature (4) & 3 & 3 & 2 & 3 & 2 & 3 \\
\rowcolor{PaleSage}  Data identification (2) & 2 & 2 & 2 & 1.5 & 2 & 1.5 \\
\rowcolor{white}     Summary stats (4) & 3 & 2 & 3 & 2 & 2 & 2 \\
\rowcolor{PaleSage}  Specification (4) & 3 & 3 & 3 & 3 & 3 & 3 \\
\rowcolor{LightGold} Critical thinking (4) & \textbf{4} & \textbf{4} & 3 & \textbf{2} & \textbf{2} & \textbf{2} \\
\rowcolor{white}     Qualified assertions (2) & 1 & 2 & 1.5 & 1.5 & 2 & 1 \\
\rowcolor{PaleSage}  Threats (2) & 1 & 2 & 1.5 & 1.5 & 1.5 & 1.5 \\
\rowcolor{white}     Bibliography (2) & 1.5 & 2 & 1.5 & 1.5 & 1 & 1 \\
\rowcolor{PaleSage}  Formatting (4) & 4 & 4 & 4 & 4 & 4 & 4 \\
\rowcolor{white}     Writing quality (4) & 3.5 & 4 & 3 & 3 & 3 & 3 \\
\rowcolor{LightGold} \textbf{Total (38)} & \textbf{32} & \textbf{34} & \textbf{29.5} & \textbf{27.5} & \textbf{27.5} & \textbf{27} \\
\rowcolor{white}     Flagged for a check-in? & no & no & no & \textbf{yes} & \textbf{yes} & \textbf{yes} \\
\end{tabular}
}
\vskip2pt
{\scriptsize\color{CoolGray} Cop.\ = Copilot (NetID) $\cdot$ Son.\ = Sonnet 5 $\cdot$ 5.6 = GPT-5.6-sol\par}
\end{column}
\begin{column}{0.40\textwidth}
\begin{tcolorbox}[keybox, title={\faSearch\quad What that table says}, top=2pt, bottom=2pt]
\scriptsize
\begin{itemize}\setlength{\itemsep}{3pt}
  \item \textbf{Seven points of spread} on one paper, one rubric --- and it \emph{straddles} me. Four came in under, one over.
  \item \textbf{Critical thinking ran 2 to 4.} The judgment criterion, exactly where you'd expect it.
  \item \textbf{Three of the five flagged this student} for a check-in. I hadn't.
  \item \textbf{Opus and Fable tied at 27.5} while disagreeing on five of twelve criteria. Same total is not agreement.
\end{itemize}
\end{tcolorbox}
\vskip3pt
\begin{tcolorbox}[quotebox, top=1pt, bottom=1pt]
\scriptsize \textbf{How I know these mean anything:} I ran this same setup first on a paper \emph{I wrote} --- 14 planted flaws, key written before anyone graded. There, all five came in \emph{above} my key. Here, four of five came in \emph{below} me.
\end{tcolorbox}
\end{column}
\end{columns}
\end{frame}
...
\begin{tcolorbox}[keybox, title={\faTools\quad On this paper}, top=2pt, bottom=2pt]
\scriptsize
\begin{itemize}\setlength{\itemsep}{3pt}
  \item \textbf{Codex} wrote the best next steps --- and handed the student the exact sentence to use instead of ``prove.'' My length.
  \item \textbf{Fable} was the most complete, and got the structure of my feedback: the name, the quoted sentences, the headers. Three times my length.
  \item \textbf{Sonnet} was fine --- but called four summaries ``genuinely good synthesis.''
  \item \textbf{Nobody} got the pivot. No ``That said,'' no closing on the positive.
\end{itemize}
\end{tcolorbox}
\end{column}
\begin{column}{0.49\textwidth}
\begin{tcolorbox}[dontbox, title={\faExclamationTriangle\quad But across everything I've graded}, top=2pt, bottom=2pt]
\scriptsize
\begin{itemize}\setlength{\itemsep}{3pt}
  \item \textbf{Codex has been the harsher grader}, and the least likely to pad a score. That part is genuinely useful.
  \item \textbf{But it reflects less.} It applies criteria in odd ways, and the feedback comes out less constructive.
  \item \textbf{And it doesn't get my voice} --- consistently the furthest from how I actually write to a student.
  \item \emph{All of which may change with the next model. This is a snapshot, not a ranking.}
\end{itemize}
\end{tcolorbox}
\end{column}
\end{columns}
...
%% ====== SLIDE: WHAT I CHANGED BEFORE SENDING ======
\begin{frame}{What I changed before any of it reached a student}
\vskip-6pt
{\footnotesize Every S26 proposal: the agent's draft against the document I actually sent.\par}
\vskip1pt
\begin{columns}[T]
\begin{column}{0.44\textwidth}
\begin{tcolorbox}[keybox, title={\faBalanceScale\quad Its score, then mine}, top=2pt, bottom=3pt]
\scriptsize
\setlength{\tabcolsep}{4pt}\renewcommand{\arraystretch}{1.08}
\begin{tabular}{c c c}
\rowcolor{SlateTeal}
\textcolor{white}{\textbf{draft}} & \textcolor{white}{\textbf{sent}} & \textcolor{white}{\textbf{change}} \\
\rowcolor{LightGold} 22.5 & 31.5 & \textbf{+9.0} \\
\rowcolor{LightGold} 24.0 & 30.5 & \textbf{+6.5} \\
\rowcolor{LightGold} 26.5 & 32.0 & \textbf{+5.5} \\
\rowcolor{white}     29.5 & 33.5 & +4.0 \\
\rowcolor{PaleSage}  30.0 & 32.0 & +2.0 \\
\rowcolor{white}     31.0 & 31.5 & +0.5 \\
\rowcolor{PaleSage}  34.5 & 34.5 & --- \\
\rowcolor{white}     36.0 & 36.0 & --- \\
\end{tabular}
\end{tcolorbox}
\vskip3pt
\begin{tcolorbox}[accentbox, top=1pt, bottom=1pt]
\scriptsize \textbf{10 of 14 raised. 0 lowered. Mean $+2.4$.} All of it at the bottom --- strong papers went out exactly as scored.
\end{tcolorbox}
\end{column}
\begin{column}{0.54\textwidth}
\begin{tcolorbox}[dontbox, title={\faCut\quad What I cut}, top=1pt, bottom=1pt, fonttitle=\bfseries\scriptsize]
\scriptsize
The grading apparatus (every time) $\cdot$ comparisons to other students (all 13) $\cdot$ references to their earlier assignments (48 $\rightarrow$ 5) $\cdot$ the closing pep talk (13 of 17) $\cdot$ ``this costs you 4 points''
\end{tcolorbox}
\vskip3pt
\begin{tcolorbox}[dobox, title={\faPlus\quad What I added}, top=1pt, bottom=1pt, fonttitle=\bfseries\scriptsize]
\scriptsize
Bad controls and mediators $\cdot$ categorical variables entered as linear $\cdot$ panel vs.\ repeated cross-section $\cdot$ what the estimation sample actually \emph{is} $\cdot$ ``we talked about this on Thursday''
\end{tcolorbox}
\vskip3pt
\begin{tcolorbox}[pipelinebox, title={\faComment\quad And the register}, top=1pt, bottom=1pt, fonttitle=\bfseries\scriptsize]
\scriptsize
3 exclamation marks across all its drafts; 29 across mine. Twice the first person. Twice the questions. Same sentence length --- I don't write shorter sentences, I write fewer of them.
\end{tcolorbox}
\end{column}
\end{columns}
\vskip2pt
%\begin{tcolorbox}[quotebox, halign=center, top=1pt, bottom=1pt]
%\scriptsize It under-scores hardest exactly where the stakes are highest --- and I never once moved a score \emph{down}.
%\end{tcolorbox}
\end{frame}
...
\begin{columns}[T]
\begin{column}{0.32\textwidth}
\begin{tcolorbox}[dobox, title={\faCheck\quad Do}, top=2pt, bottom=2pt]
\scriptsize
\begin{itemize}\setlength{\itemsep}{3pt}
\item \textbf{Make a good rubric} Transparent to students, clear for you
  \item \textbf{Split the rubric.} Countable $\rightarrow$ gate. Checkable $\rightarrow$ agent. Judgment $\rightarrow$ you.
  \item \textbf{Give it your context.} The assignment, and your own past feedback.
  \item \textbf{Test run}: Set up everything and have it test grade a few. Identify problems and tweak
  \item \textbf{De-identify first.} Mine never sees a name, so I can use whatever model grades best.
\end{itemize}
\end{tcolorbox}
\end{column}
\begin{column}{0.32\textwidth}
\begin{tcolorbox}[dontbox, title={\faExclamationTriangle\quad Expect}, top=2pt, bottom=2pt]
\scriptsize
\begin{itemize}\setlength{\itemsep}{3pt}
  \item \textbf{It can drift} --- always on the judgment lines. 
  \item \textbf{It compares.} Sending one agent on several papers would downgrade papers that were relatively worse. (Needs a fix)
  \item \textbf{Plan for your own pass.} This will take time!
  \item \textbf{Models differ, and it moves.} What was true in April wasn't true yesterday.
  \item \textbf{The checks hold.} 133 citations verified, none fabricated; the audits caught a crashed log, a 14-line do-file, a blank template.
\end{itemize}
\end{tcolorbox}
\end{column}
\begin{column}{0.32\textwidth}
\begin{tcolorbox}[keybox, title={\faBalanceScale\quad When it's worth it}, top=2pt, bottom=2pt]
\scriptsize
\begin{itemize}\setlength{\itemsep}{3pt}
  \item \textbf{Worth it} when there are many submissions, many criteria, and real mechanical checking to do.
%  \item \textbf{Not worth the plumbing} for three-question essays --- Copilot and a good rubric are enough. The rubric is the same file either way.
  \item \textbf{At 21 submissions, it wasn't clearly faster.} It was more consistent and broadened constructive suggestions. 
\end{itemize}
\end{tcolorbox}
\end{column}
\end{columns}
\vskip3pt
%\begin{tcolorbox}[accentbox, halign=center, top=1pt, bottom=1pt]
%\small Every correction I've had to make landed on a \emph{judgment} criterion. None landed on a gate or a check.
%\end{tcolorbox}
\end{frame}

%% ====== SLIDE: THE PIPELINE ====== (CUT 2026-08-26 by Emily; \iffalse ... \fi to restore)
\iffalse
...
\footnotesize  The comparison isn't AI vs.\ a perfect grader --- it's AI-plus-review vs.\ me, essay 43 of 60, at 10pm.
\end{tcolorbox}
\end{column}
\end{columns}
\vskip5pt
\begin{tcolorbox}[accentbox, top=3pt, bottom=3pt]
\small What I don't have a settled answer on: \textbf{disclosure.} Do we tell students? What do we tell them? 
\end{tcolorbox}
\end{frame}
