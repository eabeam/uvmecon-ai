...
\vskip3pt
\textbf{Keep it.} And watch it --- this is the line every grader inflated.
\end{tcolorbox}
\end{column}
\end{columns}
\end{frame}
...
%% ====== SLIDE: AND THEN A REAL PAPER ======
\begin{frame}{So I ran it again --- on a real one}
\vskip-6pt
{\footnotesize One real S26 proposal, de-identified, mid-distribution. Same rubric, three blind graders. \textbf{No answer key --- just the grade I gave it.}\par}
\vskip1pt
\begin{columns}[T]
\begin{column}{0.53\textwidth}
\begin{tcolorbox}[pipelinebox, title={\faArrowUp\quad The fake paper: everyone scored \emph{above} the key}, top=1pt, bottom=1pt, fonttitle=\bfseries\scriptsize]
\scriptsize
\ttfamily Opus +3.0\ \ Codex +3.5\ \ Fable +4.0\\
Copilot +5.5\ \ Sonnet +6.0
\end{tcolorbox}
\vskip4pt
\begin{tcolorbox}[accentbox, title={\faArrowDown\quad The real paper: all three scored \emph{below} me}, fonttitle=\bfseries\scriptsize, coltitle=DeepCharcoal, colbacktitle=LightGold!80, top=1pt, bottom=1pt]
\scriptsize
\ttfamily I sent 32\\
Sonnet 29.5 (--2.5)\ \ Opus 27.5 (--4.5)\\
Fable\ 27.5 (--4.5)
\end{tcolorbox}
\vskip3pt
\begin{tcolorbox}[quotebox, top=1pt, bottom=1pt]
\scriptsize The drift isn't a property of the \emph{model}. It's a property of the \emph{situation} --- a paper built to be middling invites padding. A real one doesn't.
\end{tcolorbox}
\end{column}
\begin{column}{0.45\textwidth}
\begin{tcolorbox}[keybox, title={\faExclamationTriangle\quad And the part that worried me}, top=2pt, bottom=2pt]
\scriptsize
\begin{itemize}\setlength{\itemsep}{3pt}
  \item \textbf{Opus and Fable both landed on 27.5} --- while disagreeing on \textbf{five of twelve criteria}. One gave the research question full marks and cut the literature; the other did the reverse.
  \item \textbf{Same total is not agreement.}
  \item \textbf{Two of the three flagged the student} for a check-in. I hadn't.
\end{itemize}
\end{tcolorbox}
\vskip3pt
%\begin{tcolorbox}[quotebox, top=1pt, bottom=1pt]
%\scriptsize One paper, three graders --- a data point, not a finding. But it's the one that made me stop trusting the totals.
%\end{tcolorbox}
\end{column}
\end{columns}
\end{frame}
...
\scriptsize
\begin{itemize}\setlength{\itemsep}{3pt}
  \item \textbf{It can drifts} --- always on the judgment lines. 
...
%  \item \textbf{Not worth the plumbing} for three-question essays --- Copilot and a good rubric are enough. The rubric is the same file either way.
  \item \textbf{At 21 submissions, it wasn't clearly faster.} It was more consistent and broadened constructive suggestiosn. 
\end{itemize}
\end{tcolorbox}
\end{column}
\end{columns}
